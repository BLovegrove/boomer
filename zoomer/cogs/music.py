...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        self.activity_idle = discord.Streaming(
            name="nothing.", url="https://tinyurl.com/lamzterms")
        ...
        bot.lavalink.add_node(cfg_lava['ip'], cfg_lava['port'],
                              cfg_lava['pwd'], cfg_lava['region'], cfg_lava['name'])
    ...
    async def hooks(self, event):
        ...
        if isinstance(event, lavalink.events.TrackEndEvent):
            ...
            if repeat_track:
                player.add(requester=cfg['bot']['id'],
                           track=repeat_track, index=0)
    ...
    async def ensure_voice(self, ctx: SlashContext) -> lavalink.DefaultPlayer or None:
        ...
        should_connect = ctx.command in (
            'play', 'zplay', 'ok', 'lofi', 'test', 'best')
        ...
        if not standard_rx.match(query):
            ...
            if "open.spotify.com" in query:
                if "playlist" in query:
                    ...
                    embed.set_author(
                        name=f"Loading spotify playlist: ",
                        url="https://tinyurl.com/boomermusic",
                        icon_url="https://i.imgur.com/dpVBIer.png"
                    )
                    embed.description = util.progress_bar(0, len(playlist['data']))
                    embed.set_footer(
                        text="Please note: This can take a long time for playlists larger than 50 songs"
                    )
                    ...
                    for index, row in playlist['data'].iterrows():
                        ...
                        tracks.append(track)
                        if len(tracks) % 1 == 0:
                            embed.description = util.progress_bar(len(tracks), len(playlist['data']))
                            await loading_msg.edit(embeds=[embed])
    ...
    def embed_list(self, player: lavalink.DefaultPlayer, page: int) -> discord.Embed:
        ...
        queue_end = queue_start + (cfg['music']['queue_page_len'] -
                                   1) if page < player.fetch('pages') else len(player.queue) - 1
        if queue_end == 0:
            queue_end += 1
        ...
        for i in range(queue_start, queue_end + 1):
            ...
            embed.add_field(
                name=textwrap.shorten(
                    f"{i + 1}. *{track.title}*", width=cfg['music']['list_char_len']),
                value=f"{play_time}",
                inline=False
            )
    ...
    @cog_ext.cog_slash(
        ...
        permissions={
            cfg['guild_id']: [
                create_permission(
                    cfg['owner_id'], SlashCommandPermissionType.USER, True)
            ]
        }
    )
